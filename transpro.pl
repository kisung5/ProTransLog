...
oracion(S0,S, T0, T):- interjeccion(Palabrainter, Traduccioninter), crear(Palabrainter, S0, S1), crear(Traduccioninter, T0, T1),
						sintagma_nominal(N, S1, S2, T1, T2, P1, D1), sintagma_proposicional(S2, S3, T2, T3, 0), persona(P1, N, D1, Per),  sintagma_verbal(N, S3, S, T3, T, Per).
...
sintagma_nominal(N, S0, S, T0, T, Palabranom, Palabradet):-  nombre(G, N, Palabranom, Traduccionnom), determinante(G, N, Palabradet, Traducciondet),
...
sintagma_proposicional(S0, S, T0, T, X):- X < 5, preposicion(Palabraprep, Traduccionprep),  nombre(G, N, Palabranom, Traduccionnom),
...
analizar_lista_oracionI([H1, H2|C1], [S|CR]):- nonvar(H2), string_concat(H1, " ", S0), string_concat(S0, H2, S), vocabularioI(S), analizar_lista_oracionI(C1, CR), !. %Si H2 existe, es decir si no es una variable.
analizar_lista_oracionI([H1|C1], [H1|CR]):- analizar_lista_oracionI(C1, CR), !.
analizar_lista_oracionI([], []):- !.

analizar_lista_oracionE([H1, H2|C1], [S|CR]):- nonvar(H2), string_concat(H1, " ", S0), string_concat(S0, H2, S), vocabularioE(S), analizar_lista_oracionE(C1, CR), !. %Si H2 existe, es decir si no es una variable.
...
traducir0(E, I):- var(E), is_question(I), sub_string(I,0,_,1,I0), string_lower(I0, Il), lista_traduccion_oracionI(Il, L), o_pregunta(E_lista, L), lista_string(E_lista, E1, " "), string_concat(E1, "?", E).
traducir0(E, I):- var(I), is_question(E), sub_string(E,0,_,1,E0), string_lower(E0, El), lista_traduccion_oracionE(El, L), o_pregunta(L, I_lista), lista_string(I_lista, I1, " "), string_concat(I1, "?", I).
traducir0(E, I):- var(E), string_lower(I, Il), lista_traduccion_oracionI(Il, L), oracion(E_lista, L), lista_string(E_lista, E, " "). %Si se desconoce la parte del espanol
traducir0(E, I):- var(I), string_lower(E, El), lista_traduccion_oracionE(El, L), oracion(L, I_lista), lista_string(I_lista, I, " "). %Si se desconoce la parte del ingles
...
translog():- write("Por favor ingrese una oracion: \n"), read(X), (((traducir(X, O), !); (traducir(O, X), !)); mapearS(O)), write("Traduccion: "), write(O), write("\n"), translog().
